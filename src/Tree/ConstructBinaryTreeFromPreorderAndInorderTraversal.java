...
public class ConstructBinaryTreeFromPreorderAndInorderTraversal {
    ...
    public TreeNode helpBuildTree(int[] preorder, int ph, int pt, int[] inorder, int ih, int it) {
        if (ph > pt || ih > it)
            return null;
        TreeNode root = new TreeNode(preorder[ph]);
        int mid = search(inorder, preorder[ph], ih, it);
        int num = mid - ih;
        root.left = helpBuildTree(preorder, ph + 1, ph + num, inorder, ih, mid - 1);
        root.right = helpBuildTree(preorder, ph + num + 1, pt, inorder, mid + 1, it);
        return root;
    }

    public int search(int[] inorder, int preValue, int ih, int it) {
        for (int i = ih; i <= it; i ++)
